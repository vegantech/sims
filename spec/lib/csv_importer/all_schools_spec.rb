...
describe CSVImporter::AllSchools do
  ...
  describe "importer"  do
    it 'should work properly' do
      ...
      @dup_person_id.special_user_groups.create!(:district_id => @district.id, :grouptype => SpecialUserGroup::ALL_STUDENTS_IN_DISTRICT)
      [@role_no_district_user_id, @should_lose_role, @should_keep_role].each do |u|
        u.special_user_groups.create!(:district_id => @district.id, :grouptype => SpecialUserGroup::ALL_SCHOOLS_IN_DISTRICT)
      end

      @district.users.update_all("updated_at = '2000-01-01'")
      @i=CSVImporter::AllSchools.new "#{Rails.root}/spec/csv/roles_base.csv",@district
      @i.import
    ...
    def check_special_user_groups u
      ...
      u.reload.special_user_groups.first.grouptype.should == SpecialUserGroup::ALL_SCHOOLS_IN_DISTRICT

...
class UserInterventions
  ...
  def to_table
    ...
    # participants = @user.intervention_participants
    # table = InterventionParticipant.report_table(:all, :only => ["role"], :conditions => ["intervention_participants.user_id = ?", @user.id])

    # return table if table.column_names.blank?

    # table.rename_columns("role" => "Role", "intervention.report_summary" => "Title / Status",

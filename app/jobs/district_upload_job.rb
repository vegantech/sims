...
class DistrictUploadJob
  ...
  def perform(upload_file, district, email = false)
    ...
    begin
      importer.import
      if email::Notifications.district_upload_results(
        importer.messages, email.presence || ::UNASSIGNED_EMAIL).deliver
      end
    rescue => e
      Rails.logger.error(
        "Job Exception #{Time.now} #{e.message} #{e.backtrace}")
      Rails.cache.write(
        "#{district.id}_import", "We're sorry, but something went wrong."\
